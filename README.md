...
### 1.1 什么是协同过滤？
利用用户与项目（如商品、内容）的历史交互数据（如点击、购买、评分），挖掘用户与用户、项目与项目之间的潜在关联，从而为用户推荐可能感兴趣的项目。它无需依赖项目的内容特征（如商品类别、文本描述）或用户的属性特征（如年龄、性别），仅通过 “用户 - 项目” 的交互关系即可实现个性化推荐，因此在缺乏额外特征的场景中极具优势。

从实现范式上，协同过滤主要分为两类：

#### 1.1.1 基于内存的 CF（如用户- based、项目 - based）：

直接利用历史交互计算用户 / 项目相似度，但难以处理大规模数据
